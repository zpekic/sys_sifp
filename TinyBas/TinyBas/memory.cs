...
namespace TinyBas
{
    ...
    {
        private ushort[] memory = new ushort[1 << 16];
        private string[] ilSource;
        private Dictionary<ushort, int> ilCodeToSourceLineDictionary = new Dictionary<ushort, int>();
        private ConsoleKeyInfo keyInfo;
        private List<char> kbdBuffer = new List<char>();
        ...
        public ushort this[ushort index]
        {
            ...
            set
            {
                switch ((index >> 12) & 0x000F)
                {
                    case 0:
                        // ROM
                        Program.Assert(false, $"Trying to write value {value:X4} to ROM address {index:X4}");
                        break;
                    case 1:
                        // Video RAM
                        memory[index] = value;
                    ...
                    case 2:
                        ...
                        else
                        {
                            // write UART data
                            memory[index] = value;
                            Console.Write((char)(value & 0x007F));
                        }
                        break;
                    default:
                        memory[index] = value;
                ...
                switch ((index >> 12) & 0x000F)
                {
                    ...
                    case 2:
                        if ((index & 0x0001) == 0)
                        {
                            ...
                            if (keyInfo != null)
                            {
                                kbdBuffer.Add(keyInfo.KeyChar);
                            }
                            if (kbdBuffer.Count == 0)
                            {
                                return 0x0001;  // TDRE (always ready to transmit)
                            }
                            else
                            {
                                return 0x0003;  // RDRE and TDRE (something received, ready to transmit)
                            }
                        }
                        else
                        {
                            ...
                            if (kbdBuffer.Count == 0)
                            {
                                return 0x0000;  // no character
                            }
                            else
                            {
                                ushort ascii = (ushort)(kbdBuffer.ElementAt(0) & 0x007F);
                                kbdBuffer.RemoveAt(0);
                                return ascii;
                            }
                        }
                    case 15:
        ...
        public void TraceILCode(ushort ilOperationAddress)
        {
            Program.Assert(ilCodeToSourceLineDictionary.ContainsKey(ilOperationAddress), $"{ilOperationAddress:X4} does not point to a valid IL instruction");

            Console.WriteLine(ilCodeToSourceLineDictionary[ilOperationAddress]);
        }

        public ushort LoadIL(ushort ilBaseAddress, FileInfo ilSourceFileInfo)
        {
            ushort offset;
            uint address = 0;

            if (ilSourceFileInfo != null)
            {
                // TODO: real "compile" of the updated IL Source
            }
            else
            {   // "fake compile" - just extract binary from the defauls
                ilSource = DefaultIL();
                // now parse the source code into memory
                for (int i = 0; i < ilSource.Length; i++)
                {
                    string[] line = ilSource[i].Split(';');
                    Program.Assert(line.Length > 0, $"Unparsable IL line {i} '{ilSource[i]}'");
                    string[] hexData = line[0].Trim().Split(' ');
                    switch (hexData.Length)
                    {
                        case 0:
                            Program.Assert(line.Length == 2, $"Unparsable IL line {i} '{ilSource[i]}'");
                            break;
                        case 1:
                            break;
                        case 2:
                        case 3:
                            offset = ushort.Parse(hexData[0], System.Globalization.NumberStyles.HexNumber);
                            Program.Assert((offset >= 0) && (offset <= 65535), "Bad value");
                            for (int j = 0; j < hexData[1].Length; j += 2)
                            {
                                ushort word = ushort.Parse(hexData[1].Substring(j, 2), System.Globalization.NumberStyles.HexNumber);
                                Program.Assert((word >= 0) && (word <= 65535), "Bad offset address");
                                address = (uint)ilBaseAddress + (uint)offset;
                                Program.Assert((address >= 0) && (address <= 65535), "Bad absolute address");
                                ilCodeToSourceLineDictionary.Add((ushort)address, i);
                                memory[(ushort)address] = word;
        ...
        public ushort Decaps(ushort b)
        {
            int nx;

            for (nx = 0; nx < 32; nx++) memory[b + nx] = '\0';     /* fill caps table.. */
            for (nx = 32; nx < 127; nx++) memory[b + nx] = (char)nx;
            for (nx = 65; nx < 91; nx++) memory[b + nx + 32] = (char)nx;
            memory[b + 9] = ' ';
            memory[b + 10] = '\r';
            memory[b + 13] = '\r';
            memory[b + 127] = '\0';
        ...
        private void strcpy(List<string> s, string line)
        {
            strcat(s, line);
        }

        private void strcat(List<string> s, string line)
        {
            s.Add(line);
        }
    }
